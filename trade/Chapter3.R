...
data.model <- specifyModel(T.ind(GSPC) ~ Delt(Cl(GSPC),k=1:10) + myATR(GSPC) + myADX(GSPC) +    myEMV(GSPC) + myVolat(GSPC)  + myMACD(GSPC) + mySAR(GSPC) + runMean(Cl(GSPC)) )
...
registerDoMC(core = 2)
set.seed(100)
# indxPre <- createTimeSlices(Tdata.train[ , 'T.ind.GSPC'],
#                          horizon = 1000,
#                          initialWindow = 6000)
ctrl <- trainControl(method = "timeslice",
                     ...
                     horizon = 1000,
                     fixedWindow = TRUE)
svB <- train(T.ind.GSPC ~ ., data = Tdata.train,
             method = "svmRadial",
             preProc = c("center", "scale"),
             tuneLength = 3,
             trControl = ctrl)

s.predsB <- predict(svB,Tdata.train[1001:2000,])
sigs.svmB <- trading.signals(s.predsB,0.1,-0.1)
sigs.PR(sigs.svmB,true.sigs)
...
marsGrid <- expand.grid(.degree = 1:3, .nprune = 2:17)
eB <- train(T.ind.GSPC ~ ., data = Tdata.train,
             method = "earth",
             preProc = c("center", "scale"),
             tuneGrid = marsGrid,
             trControl = ctrl)

e.predsB <- predict(eB,Tdata.train[1001:2000,])
sigs.eB <- trading.signals(e.predsB,0.1,-0.1)
sigs.PR(sigs.eB,true.sigs)
